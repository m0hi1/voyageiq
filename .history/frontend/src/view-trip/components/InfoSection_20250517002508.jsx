    ...
    <div className="p-4 sm:p-6 bg-white rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-300 ease-in-out">
      ...
      <div className="relative mb-6">
        <img
          ...
          alt={`Image of ${userSelection?.location || 'the destination'}`}
        />
      </div>
      ...
      <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <h1 className="text-3xl sm:text-4xl font-bold text-slate-800 break-words">
          {userSelection?.location || 'Your Amazing Trip'}
        </h1>
        <Button
          variant="outline"
          size="lg"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <DetailItem
          ...
          label={`${userSelection?.noOfDays || 'N/A'} Days`}
          value="Duration"
        />
        <DetailItem
          icon={<CircleDollarSign className="h-6 w-6 text-green-600" />}
          label={`${userSelection?.budget || 'N/A'} Budget`}
          value="Est. Cost"
        />
        <DetailItem
          icon={<Users className="h-6 w-6 text-purple-600" />}
          label={`${userSelection?.noOfTravellers || 'N/A'} Traveler(s)`}
          value="Group Size"
        />
      </div>
    </div>
